...
import net.runelite.client.plugins._Viheiser.ViUtilities.api.utilities.menuentries.ObjectMenuEntries;
...
public class ObjectUtils {
    ...
    @Inject
    private ObjectMenuEntries objectMenuEntries;

    public void interactWithObject(int gameObjectId){
        GameObject gameObject = getNearestGameObject(gameObjectId);

        if (gameObject != null) {
            invokeInteractions.invokeMenuAction(objectMenuEntries.firstOption(gameObject));
        }
    }
    ...
    public GameObject getNearestGameObject(Collection<Integer> gameObjectIds) {
        GameObject nearestGameObject = findNearestGameObject(gameObjectIds);
    ...
    private GameObject findNearestGameObject(Collection<Integer> gameObjectIds) {
        WorldPoint playerLocation = client.getLocalPlayer().getWorldLocation();
        return getGameObjects().stream()
                .filter(gameObject -> gameObjectIds.contains(gameObject.getId()))
                .min(Comparator.comparingInt(gameObject -> calculateDistance(playerLocation, gameObject.getWorldLocation())))
                .orElse(null);
    }
}
